...
ssh user@gpu-server "echo 'connected'"  # Verify IP works
...
❌ **Wrong** (guessing):
```
ssh user@gpu-server-old  # WRONG IP from stale memory
```
...
# → Returns: "gpu-server-old" (STALE!)

# Step 2: Verify in TOOLS.md
cat ~/.openclaw/workspace/TOOLS.md | grep "GPU Server"
# → Shows: "user@gpu-server" (CURRENT!)

# Step 3: Test connection
ssh user@gpu-server "hostname"
# → Works!
...
2. **Write to WAL**
   ...
   uv run python skills/agent-self-governance/scripts/wal.py append main \
     ...
     "Failed to verify GPU server IP. Used gpu-server-old (stale) instead of gpu-server (current)."
   ```
